...
# Cloud Run v2 services. Ingress is restricted to the internal Application LB
# so the *.run.app URL is unreachable from outside the VPC.
resource "google_cloud_run_v2_service" "mcp" {
  ...
  # The agent reaches each service via the internal LB at `https://<name>.<domain>`
...
# except by holders of an OIDC token for the invoker SA. Per-agent authorization
# is still enforced upstream at the Agent Gateway via `roles/iap.egressor`
# (see scripts/grant_agent_mcp_egress.sh).
#
