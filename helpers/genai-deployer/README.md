...
- See `0-bootstrap` README for additional IAM [requirements](../../0-bootstrap/README.md#prerequisites) on the user deploying the Genai.
...
- Create a directory in the file system to host the Cloud Source repositories the will be created and a copy of the terraform Google enterprise genai.
...
- Variable `genai_code_path` is the full path to `terraform-google-enterprise-genai` directory.
- See the READMEs for the stages for additional information:
  ...
  - [3-networks-svpc](https://github.com/GoogleCloudPlatform/terraform-google-enterprise-genai/blob/master/3-networks-svpc)
...
By default the Genai regional resources are deployed in `us-west1` and `us-central1` regions and multi-regional resources are deployed in the `US` multi-region.

In addition to the variables declared in the file `global.tfvars` for configuring location, there are two locals, `default_region1` and `default_region2`, in each one of the environments (`production`, `nonproduction`, and `development`) in the network step (`3-networks-svpc`).
