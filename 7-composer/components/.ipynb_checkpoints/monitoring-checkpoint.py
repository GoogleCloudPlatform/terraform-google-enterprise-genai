...
def create_monitoring(
    ...
    list_monitors = aiplatform.ModelDeploymentMonitoringJob.list(filter=f'display_name="{monitoring_name}"', project=project_id)
    if len(list_monitors) == 0:
        ...
        """
        for feature in schema:
            if feature.name == "income_bracket":
                continue
            yaml += f"""- {feature.name}
        """
        with open("monitoring_schema.yaml", "w") as f:
            f.write(yaml)
