...
To satisfy the required permissions, the following predefined roles can be granted:
...
| kms\_key\_name | The KMS key to be used on the keyring, if not specified will use the default key created by the standalone example | `string` | `""` | no |
...
| log\_bucket | Bucket to store logs from the created bucket. This is the Env-level Log Bucket created on 2-environments. | `string` | n/a | yes |
