...
|------|-------------|------|---------|:--------:|
| agent\_gateway\_authz\_fail\_open | Allow traffic when authz extensions fail | `bool` | `false` | no |
| agent\_gateway\_dns\_peering\_config | Optional DNS peering for the Agent Gateway | <pre>object({<br>    domains        = list(string)<br>    target_project = optional(string)<br>    target_network = optional(string)<br>  })</pre> | `null` | no |
| agent\_gateway\_iap\_iam\_enforcement\_mode | IAP IAM enforcement mode: null or DRY\_RUN | `string` | `null` | no |
| agent\_gateway\_name | Name of the Agent Gateway resource | `string` | `"agent-gateway"` | no |
| agent\_gateway\_subnet\_cidr | CIDR for the Agent Gateway dedicated subnet. Must be within RFC1918, min /28. If inside 10.0.0.0/8, must not overlap 10.0.0.0/24, 10.0.1.0/24, or 10.0.2.0/24 (Agent Gateway egress restrictions). | `string` | `"10.20.0.0/28"` | no |
...
| agent\_registry\_google\_apis | Map of Google API IDs to register in Agent Registry | `map(string)` | <pre>{<br>  "agentregistry": "Agent Registry",<br>  "aiplatform": "Vertex AI Platform",<br>  "cloudresourcemanager": "Cloud Resource Manager",<br>  "discoveryengine": "Discovery Engine",<br>  "iamcredentials": "IAM Credentials",<br>  "iap": "Identity-Aware Proxy",<br>  "logging": "Logging",<br>  "modelarmor": "Model Armor",<br>  "monitoring": "Monitoring",<br>  "oauth2": "OAuth2",<br>  "telemetry": "Telemetry",<br>  "trace": "Trace"<br>}</pre> | no |
| billing\_account | The ID of the billing account to associate this project with. | `string` | n/a | yes |
| bucket\_force\_destroy | When deleting a bucket, this boolean option will delete all contained objects. If false, Terraform will fail to delete buckets which contain objects. | `bool` | `true` | no |
| cloud\_source\_artifacts\_repo\_name | Name to give the Cloud Source repository for artifacts. | `string` | `"publish-artifacts"` | no |
| cloudbuild\_bucket\_name | Override the Cloud Build source bucket name. Defaults to <project\_id>\_cloudbuild | `string` | `null` | no |
| deletion\_policy | Whether Terraform will be prevented from destroying the resource. Defaults to DELETE. When a 'terraform destroy' or 'terraform apply' would delete the resource, the command will fail if this field is set to "PREVENT" in Terraform state. When set to "ABANDON", the command will remove the resource from Terraform management without updating or deleting the resource in the API. When set to "DELETE", deleting the resource is allowed. | `string` | n/a | yes |
| dns\_zone\_domain | The domain name for the public DNS zone (must end with a dot, e.g., 'example.com.'). | `string` | `null` | no |
...
| enable\_bigquery\_sink | Whether to create a BigQuery dataset and export agent platform logs to it. | `bool` | `false` | no |
| enable\_model\_armor\_gemini\_enterprise | Enable a multi-region Model Armor template for Gemini Enterprise | `bool` | `false` | no |
| enable\_model\_armor\_mcp\_floor\_setting | Enable Model Armor floor setting for MCP server protection | `bool` | `true` | no |
| enable\_model\_armor\_vertex\_ai | Enable Model Armor integration with Vertex AI | `bool` | `true` | no |
| encrypt\_gcs\_bucket\_tfstate | Encrypt the bucket used for storing Terraform state files in the seed project. | `bool` | `true` | no |
| kms\_prevent\_destroy | If set to true, delete the KMS key ring and keys when destroying the module; otherwise, destroying the module will fail if KMS keys are present. | `bool` | `false` | no |
| mcp\_internal\_dns\_zone | Private DNS zone hosting <service>.<domain> A records for the MCP Cloud Run services. | <pre>object({<br>    name   = optional(string, "mcp-server-internal")<br>    domain = string<br>  })</pre> | `null` | no |
| mcp\_services | Map of MCP service name to deployment configuration. | <pre>map(object({<br>    image              = string<br>    container_port     = optional(number, 8080)<br>    otel_service_name  = optional(string)<br>    min_instance_count = optional(number, 0)<br>    max_instance_count = optional(number, 3)<br>    cpu                = optional(string, "1")<br>    memory             = optional(string, "512Mi")<br>    env                = optional(map(string), {})<br>  }))</pre> | `{}` | no |
...
| model\_armor\_gemini\_enterprise\_location | Multi-region location for the Gemini Enterprise template | `string` | `"us"` | no |
| model\_armor\_gemini\_enterprise\_template\_id | ID for the Gemini Enterprise Model Armor template | `string` | `"gemini-enterprise-safety-template"` | no |
| model\_armor\_malicious\_uri\_enforcement | Malicious URI filter enforcement setting | `string` | `"ENABLED"` | no |
| model\_armor\_pi\_jailbreak\_confidence | PI and jailbreak filter confidence level | `string` | `"LOW_AND_ABOVE"` | no |
| model\_armor\_pi\_jailbreak\_enforcement | PI and jailbreak filter enforcement setting | `string` | `"ENABLED"` | no |
| model\_armor\_pii\_types | Info types to deidentify in Model Armor. | `list(string)` | <pre>[<br>  "US_SOCIAL_SECURITY_NUMBER",<br>  "CREDIT_CARD_NUMBER",<br>  "PHONE_NUMBER",<br>  "EMAIL_ADDRESS",<br>  "PASSPORT",<br>  "DATE_OF_BIRTH",<br>  "MEDICAL_RECORD_NUMBER",<br>  "IP_ADDRESS",<br>  "STREET_ADDRESS"<br>]</pre> | no |
| model\_armor\_rai\_filters | RAI filter configurations. | <pre>list(object({<br>    filter_type      = string<br>    confidence_level = string<br>  }))</pre> | <pre>[<br>  {<br>    "confidence_level": "MEDIUM_AND_ABOVE",<br>    "filter_type": "HATE_SPEECH"<br>  },<br>  {<br>    "confidence_level": "MEDIUM_AND_ABOVE",<br>    "filter_type": "HARASSMENT"<br>  },<br>  {<br>    "confidence_level": "MEDIUM_AND_ABOVE",<br>    "filter_type": "SEXUALLY_EXPLICIT"<br>  }<br>]</pre> | no |
| model\_armor\_request\_template\_id | ID for the request-side Model Armor template | `string` | `"agw-request-template"` | no |
| model\_armor\_response\_template\_id | ID for the response-side Model Armor template | `string` | `"agw-response-template"` | no |
| model\_armor\_sdp\_enforcement | Sensitive Data Protection filter enforcement setting | `string` | `"ENABLED"` | no |
| model\_armor\_vertex\_ai\_cloud\_logging | Enable Cloud Logging for Vertex AI Model Armor | `bool` | `true` | no |
| model\_armor\_vertex\_ai\_inspect\_only | Inspect only mode for Vertex AI Model Armor | `bool` | `true` | no |
...
| project\_name | The GCP project name to create | `string` | `""` | no |
| project\_prefix | Name prefix to use for projects created. Should be the same in all steps. Max size is 3 characters. | `string` | `"prj"` | no |
| proxy\_subnet\_cidr | CIDR range for the proxy-only subnet | `string` | `"10.9.0.0/24"` | no |
| psc\_interface\_dns\_zone | Private DNS zone for PSC Interface DNS peering. | <pre>object({<br>    name   = optional(string, "psc-interface-dns-zone")<br>    domain = string<br>  })</pre> | `null` | no |
| psc\_interface\_subnet\_cidr | CIDR for the PSC Interface subnet | `string` | `"10.11.0.0/28"` | no |
...
| terraform\_service\_account | The email address of the service account that will run the Terraform code. List of IAM granted droles: discoveryengine.admin always; modelarmor.admin and modelarmor.floorSettingsAdmin for model armor; aiplatform.user when for agent engine (e.g. ["user:admin@example.com"]) | `string` | n/a | yes |
...
| agent\_mcp\_invoker\_email | Email of the SA agents impersonate when invoking MCP Cloud Run services. Pass to deploy\_agent.py via --mcp-invoker-sa or $MCP\_INVOKER\_SA\_EMAIL. Null when enable\_agent\_engine is false. |
...
| bucket\_mortgage\_terraform\_state | The GCS bucket for Terraform state |
| mcp\_internal\_dns\_domain | Domain name (with trailing dot) of the MCP servers private DNS zone |
| mcp\_internal\_dns\_names | Map of MCP service key to its private DNS name (<service>.<domain>). Null when enable\_private\_network = false (no MCP private zone is provisioned). |
| mcp\_internal\_dns\_zone\_name | Cloud DNS managed zone name for the MCP servers private DNS |
| mcp\_lb\_ip\_address | Internal IP address that fronts the MCP services LB |
...
| mcp\_service\_urls | Map of MCP service key to Cloud Run *.run.app URL |
...
| model\_armor\_gemini\_enterprise\_template\_name | Full resource name of the Gemini Enterprise Model Armor template (for Discovery Engine REST API) |
...
| model\_armor\_vertex\_ai\_service\_account | AI Platform service agent email granted Model Armor access |
| nat\_gateway\_name | Cloud NAT gateway name |
| nat\_router\_name | Cloud Router name |
| network\_id | VPC network ID |
| network\_name | VPC network name |
| network\_self\_link | The self-link of the VPC network |
| primary\_subnet\_id | The self-link of the primary subnet |
| primary\_subnet\_name | Name of the primary subnet |
| project\_id | The created GCP project ID for Mortgage Agent |
| project\_number | The created GCP project number |
| proxy\_subnet\_id | ID of the proxy-only subnet for internal load balancers |
| psc\_interface\_dns\_domain | Domain name for PSC Interface DNS peering (ends with a dot) |
| psc\_interface\_network\_attachment\_id | Full resource ID of the PSC Interface network attachment |
| psc\_subnet\_id | ID of the Private Service Connect subnet |
| psc\_subnet\_self\_link | Self link of the Private Service Connect subnet |
| region | The GCP region for resources |
| regional\_certificate\_name | Name of the regional Google-managed certificate. Null unless both enable\_private\_network and enable\_certificate\_manager are true. |
| subnet\_id | All VPC subnets ID |
| subnets | All subnets |
| subnets\_summary | The summary of all subnets with name, ip and id |
